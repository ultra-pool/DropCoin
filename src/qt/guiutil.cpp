...
{
    // return if URI is not valid or is no dropcoin: URI
...
{
    // Convert dropcoin:// to dropcoin:
    //
    //    Cannot handle this later, because dropcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open dropcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DropCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for DropCoin.lnk
...
{
    return GetAutostartDir() / "dropcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a dropcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DropCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the dropcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add dropcoin app to startup item list
